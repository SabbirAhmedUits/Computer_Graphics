...
int main() {
    ...
    int ball_y = 250;             // Set initial ball's y-coordinate
    int ball_radius = 20;        // Radius of the ball
    int speed = 1;               // Speed of the ball
    int direction = 1;           // 1 for down, -1 for up
    int color_index = 1;         // Color index for the ball

    // Bouncing counter
    int bounces = 0;
    int max_bounces = 100;  // Stop the ball after 10 bounces
    ...
    while (true) {
        ...
        if (ball_y + ball_radius >= getmaxy()-100 || ball_y - ball_radius <= 0) {
            direction = -direction;
